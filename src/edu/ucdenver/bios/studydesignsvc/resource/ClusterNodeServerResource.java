...
/**
 * Server Resource class for handling requests for the Cluster Node object. 
...
{	
     ...
     */
	@Get("json")
	public List<ClusterNode> retrieve(byte[] uuid) 
	{
		...
		catch(StudyDesignException sde)
		{
			...
			if(studyDesignManager!=null)
			{
				...
				catch(BaseManagerException re) {clusterNodeList = null;}					
			}
			clusterNodeList = null;
		}								
     ...
     */
	@Post("json")
	public List<ClusterNode> create(byte[] uuid,List<ClusterNode> clusterNodeList) 
	{		
		...
		try
		{
			...
			{				
				...
				/* ----------------------------------------------------
    			 * Set reference of ClusterNode Object to Study Design object 
    			 * ----------------------------------------------------*/
     ...
     */
	@Put("json")
	public List<ClusterNode> update(byte[] uuid,List<ClusterNode> clusterNodeList) 
	{				
     ...
     */
	@Delete("json")
	public List<ClusterNode> remove(byte[] uuid) 
	{
		List<ClusterNode> clusterNodeList = null;
		StudyDesign studyDesign = null;
		if(uuid==null)
		...
		try
		{
			...
			studyDesignManager.commit();
			/* ----------------------------------------------------
			 * Remove existing ClusterNode objects for this object 
			 * ----------------------------------------------------*/
			if(clusterNodeList!=null)
			{
				clusterNodeManager = new ClusterNodeManager();
				clusterNodeManager.beginTransaction();
					clusterNodeList = clusterNodeManager.delete(uuid,clusterNodeList);
					clusterNodeManager.commit();
			}
		}
		catch (BaseManagerException bme)
     ...
     */
	@Delete("json")
	public List<ClusterNode> removeFrom(StudyDesign studyDesign) 
	{
		boolean flag;	
        ...
        {                    			
        	clusterNodeManager = new ClusterNodeManager();
        	clusterNodeManager.beginTransaction();
        		clusteringTree=clusterNodeManager.delete(studyDesign.getUuid(),studyDesign.getClusteringTree());
        	clusterNodeManager.commit();        	       
        }
        catch (BaseManagerException bme)
        {
            ...
            if (clusterNodeManager != null) try { clusterNodeManager.rollback(); } catch (BaseManagerException e) {}
            clusteringTree = null;           
        }
