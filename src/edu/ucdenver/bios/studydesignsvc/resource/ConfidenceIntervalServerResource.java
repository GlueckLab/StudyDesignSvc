...
public class ConfidenceIntervalServerResource extends ServerResource implements ConfidenceIntervalResource
{
	...
	{			
		...
		{			
			...
			confidenceInterval = studyDesign.getConfidenceIntervalDescriptions();			
			/*if(uuidFlag)
			{			
     ...
     * 
     * @param uuid : byte[]
     * @param confidenceInterval : ConfidenceIntervalDescription
     * @return ConfidenceIntervalDescription
     */
	@Post("json")
	...
	{		
		...
		try
		{
			...
			studyDesignManager.commit();
			/* ----------------------------------------------------
			 * Remove existing Confidence Interval for this object 
			 * ----------------------------------------------------*/
			if(uuidFlag && studyDesign.getConfidenceIntervalDescriptions()!=null)
				removeFrom(studyDesign);	
			/* ----------------------------------------------------
			 * Save new Confidence Interval object 
			 * ----------------------------------------------------*/
			if(uuidFlag)
			{		
				confidenceIntervalManager = new ConfidenceIntervalManager();
	...
	public ConfidenceIntervalDescription update(byte[] uuid,ConfidenceIntervalDescription confidenceInterval) 
	{
		...
		try
		{
			 ...
			 * Save new Confidence Interval object 
			 * ----------------------------------------------------*/
			ConfidenceIntervalDescription interval = studyDesign.getConfidenceIntervalDescriptions();
	...
	public ConfidenceIntervalDescription remove(byte[] uuid) 
	{
        ...
        try
        {
            ...
            {          				
            	confidenceIntervalManager = new ConfidenceIntervalManager();
            	confidenceIntervalManager.beginTransaction();
            		confidenceInterval=confidenceIntervalManager.delete(uuid,studyDesign.getConfidenceIntervalDescriptions());
            	confidenceIntervalManager.commit();
            	/* ----------------------------------------------------
    			 * Set reference of Confidence Interval Object to Study Design object 
    			 * ----------------------------------------------------*/
	...
	public ConfidenceIntervalDescription removeFrom(StudyDesign studyDesign) 
	{
        ...
        {                    			
        	confidenceIntervalManager = new ConfidenceIntervalManager();
        	confidenceIntervalManager.beginTransaction();
        		confidenceInterval=confidenceIntervalManager.delete(studyDesign.getUuid(),studyDesign.getConfidenceIntervalDescriptions());
        	confidenceIntervalManager.commit();
        	/* ----------------------------------------------------
			 * Set reference of Confidence Interval Object to Study Design object 
			 * ----------------------------------------------------*/
